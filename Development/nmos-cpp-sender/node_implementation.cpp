...
namespace impl
{
    ...
    namespace fields
    {
        ...
        const web::json::field_as_value_or frame_rate{U("frame_rate"), web::json::value_of({{nmos::fields::numerator, 25},
...
void node_implementation_thread(nmos::node_model &model, slog::base_gate &gate_)
{
    ...
    {
        if (nmos::details::wait_for(model.shutdown_condition, lock, std::chrono::milliseconds(milliseconds), [&]
    ...
    // example node
    {
        ...
        if (!insert_resource_after(delay_millis, model.node_resources, std::move(node), gate))
            return;
    }
    ...
    // example device
    {
        ...
        if (0 <= nmos::fields::events_port(model.settings))
            boost::range::push_back(sender_ids, impl::make_ids(seed_id, nmos::types::sender, impl::ports::ws, how_many));
        auto receiver_ids = impl::make_ids(seed_id, nmos::types::receiver, impl::ports::all, how_many);
        if (!insert_resource_after(delay_millis, model.node_resources, nmos::make_device(device_id, node_id, sender_ids, receiver_ids, model.settings), gate))
            return;
    }
    ...
    */
    ...
    for (int index = 0; 0 <= nmos::fields::events_port(model.settings) && index < how_many; ++index)
    {
        for (const auto &port : impl::ports::ws)
        {
            ...
            if (impl::ports::extra == port)
            {
                ...
                events_state = nmos::make_events_string_state({source_id, flow_id}, U("/testTopic/thisIsATest,2.1"));
            }
            else if (impl::ports::exposure == port)
            {
                event_type = impl::exposure;
                events_type = nmos::make_events_number_type({-200, 10}, {1000, 10}, {1, 10}, U("exposure"));
            ...
            if (!insert_resource_after(delay_millis, model.node_resources, std::move(source), gate))
                return;
            if (!insert_resource_after(delay_millis, model.node_resources, std::move(flow), gate))
                return;
            if (!insert_resource_after(delay_millis, model.node_resources, std::move(sender), gate))
                return;
            if (!insert_resource_after(delay_millis, model.connection_resources, std::move(connection_sender), gate))
                return;
            if (!insert_resource_after(delay_millis, model.events_resources, std::move(events_source), gate))
                return;
        }
    }
    ...
    model.shutdown_condition.wait(lock, [&]
                                  { return model.shutdown; });
    /*
        ...
        if (rtp_sender_ids.end() != boost::range::find(rtp_sender_ids, id_type.first))
        {
            ...
            if (smpte2022_7)
                nmos::details::resolve_auto(transport_params[1], nmos::fields::source_ip, [&]
                                            { return web::json::back(nmos::fields::constraint_enum(constraints.at(1).at(nmos::fields::source_ip))); });
            nmos::details::resolve_auto(transport_params[0], nmos::fields::destination_ip, []
                                        { return value::string(U("239.255.255.0")); });
            if (smpte2022_7)
                nmos::details::resolve_auto(transport_params[1], nmos::fields::destination_ip, []
                                            { return value::string(U("239.255.255.1")); });
        ...
        else if (rtp_receiver_ids.end() != boost::range::find(rtp_receiver_ids, id_type.first))
        {
            ...
            if (smpte2022_7)
                nmos::details::resolve_auto(transport_params[1], nmos::fields::interface_ip, [&]
                                            { return web::json::back(nmos::fields::constraint_enum(constraints.at(1).at(nmos::fields::interface_ip))); });
            // lastly, apply the specification defaults for any properties not handled above
            nmos::resolve_rtp_auto(id_type.second, transport_params);
        }
...
nmos::events_ws_message_handler make_node_implementation_events_ws_message_handler(const nmos::node_model &model, slog::base_gate &gate)
{
    ...
    return nmos::experimental::make_events_ws_message_handler(
        model, [receiver_ids, &gate](const nmos::resource &receiver, const nmos::resource &connection_receiver, const web::json::value &message)
        {
